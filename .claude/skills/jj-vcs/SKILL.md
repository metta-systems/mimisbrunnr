...
2. **Verify operations with `jj st --no-pager`** after mutations (`squash`, `abandon`, `rebase`, `restore`) to confirm the operation succeeded.
...
Validate that you're on a blank revision with `jj st --no-pager`. If you are not, you should type:
...
NEVER abandon commits without asking an explicit confirmation from the user. Proceed only if answer is unambiguously allowing the operation.
...
| View status | `jj st --no-pager` |
| View log | `jj log --no-pager` |
| View diff | `jj diff --git --no-pager` |
